...
:ref:`change data source dialog <dialog-change-data-source>`. This dialog
...
:ref:`dialog-batch-mode`. This dialog allows you to start an analysis in batch
...
for each species separately and the results are displayed in a summary report.
...
The `Change Data Source` button opens the :ref:`dialog-change-data-source`.
...
* CSV files exported from the Microsoft Access SETL database. The CSV
  ...
  :ref:`dialog-loc-selection` will be updated with the new data.

* XLS files exported from Microsoft Excel. One file for each of the four
  tables: SETL_localities, SETL_plates, SETL_records, and SETL_species. These
  files should have the same format as the CSV files exported by Microsoft
  Access and must not contain a header row.
...
* The remote SETL database. The SETL database server has not been implemented
  yet, so this functionality is not functional yet. The idea is to move the
  data from the Microsoft Access database to a PostgreSQL/MySQL database.

  This dialog should allow you to enter the information needed to
  connect to the database server (i.e. the server address and a port number),
  Pressing the OK button should load the locations and species data. A progress
  dialog is shown while the data is being loaded. Once the data has been
  loaded, the :ref:`dialog-loc-selection` will be updated with the new data.
